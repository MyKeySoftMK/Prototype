...
Public Class FtpClientFuncs
    ...
    Public Sub New()
        ...
        NLOGLOGGER.Debug("Start: FtpClientFuncs")
    End Sub
    ...
    ''' </summary>
    ''' <returns></returns>
    Public Property FTPClnt As FtpClient
    ...
    Public WriteOnly Property FTP_LOGDATA As FtpLoggingData
        Set(value As FtpLoggingData)
            ...
            If FTPClnt Is Nothing Then
                FTPClnt = New FtpClient
            End If

            If value.FTP_LOGENABLED = 1 Then


                FTPClnt.Config.LogToConsole = True
                FTPClnt.Config.LogUserName = (value.FTP_LOGHIDE = 1)
                FTPClnt.Config.LogPassword = (value.FTP_LOGHIDE = 1)
                FTPClnt.Config.LogHost = (value.FTP_LOGHIDE = 1)
                FTPClnt.Config.LogDurations = True

                If FTPClnt.Logger Is Nothing Then
                    _LoggingProvider = New NLogLoggerProvider
                    _FtpLogger = _LoggingProvider.CreateLogger(GetType(FtpClientFuncs).FullName)
                    FTPClnt.Logger = New FtpLogAdapter(_FtpLogger)
                End If

            Else

                FTPClnt.Logger = Nothing
            End If
        ...
        Try
            ...
            If FTPClnt Is Nothing Then
                ...
                With FTPClnt
                    ...
                    .Config.DataConnectionType = FtpDataConnectionType.EPSV

                End With

            End If

            If FTPClnt.IsConnected = False Then
                FTPClnt.Connect()
            End If

            NLOGLOGGER.Info("Connection is successfull")
            NLOGLOGGER.Debug("Connection Type:" & FTPClnt.ConnectionType)
            NLOGLOGGER.Debug("Internet Protocol:" & FTPClnt.InternetProtocol)
            NLOGLOGGER.Debug("Use Encryption:" & FTPClnt.IsEncrypted)
            NLOGLOGGER.Debug("Server:" & FTPClnt.ServerOS & "(" & FTPClnt.ServerType & ")")
            NLOGLOGGER.Debug("Server System:" & FTPClnt.SystemType)
            NLOGLOGGER.Debug("Endpoints: " & FTPClnt.SocketLocalEndPoint.ToString & " (local) / " & FTPClnt.SocketRemoteEndPoint.ToString & " (remote)")
    ...
    Public Function GetListOfFtpFiles(DirectoryName As String, Optional FileNameExtension As String = "") As List(Of String)
        ...
        FtpResult = FTPClnt.GetListing(DirectoryName, FtpListOption.AllFiles)
    ...
    Public Function GetNewestFtpFile(DirectoryName As String, Optional FileNameExtension As String = "") As List(Of String)
        ...
        If FtpResult.Length = 0 Then
            FTPClnt.SetWorkingDirectory(DirectoryName)
            FtpResult = FTPClnt.GetListing("", FtpListOption.AllFiles)
        End If
    ...
    Public Function DownloadFtpFile(FtpDirectory As String, FileName As String, LocalDirectory As String) As Boolean
        ...
        Try
            ...
            If FTPClnt.DownloadFile(LocalDirectory & "\" & FileName, FtpDirectory & "/" & FileName, FtpLocalExists.Overwrite) = FtpStatus.Success Then
    ...
    Public Function DownloadFtpFiles(FtpDirectory As String, FileNames As List(Of String), LocalDirectory As String) As Boolean
        ...
        Try
            ...
            If FileNames.Count > 0 Then
                ...
                If FTPClnt.DownloadFiles(LocalDirectory & "\", _ftpList, FtpLocalExists.Overwrite).Count = _ftpList.Count Then
    ...
    Public Function GetFtpFileDate(FileName As String) As Date
        ...
        Try
            ...
            result = FTPClnt.GetModifiedTime(FileName)
    ...
    Public Function UploadFtpFile(FtpDirectory As String, FileName As String, LocalDirectory As String) As Boolean
        ...
        Try
            FTPClnt.UploadFile(LocalDirectory & "\" & FileName, FtpDirectory & "/" & FileName, FtpRemoteExists.Overwrite)
            result = True
        Catch ex As Exception
    ...
    Public Function MoveFile(FileName As String, SourceDirectory As String, DestinationDirectory As String,
        ...
        Try
            ...
            If NewFileName.Length > 0 Then
                FTPClnt.MoveFile(SourceDirectory & "/" & FileName, DestinationDirectory & "/" & NewFileName, FtpRemoteExists.Overwrite)
                'FTPClnt.Rename(SourceDirectory & "/" & FileName, DestinationDirectory & "/" & NewFileName)
            Else
                FTPClnt.MoveFile(SourceDirectory & "/" & FileName, DestinationDirectory & "/" & FileName, FtpRemoteExists.Overwrite)
    ...
    Public Function ConnectWithDownload(FtpServer As FtpServer, DirectoryName As String, LocalDirectory As String,
        ...
        Try
            NLOGLOGGER.Debug("ConnectWithDownload (FTP) ...")
    ...
    Public Function ConnectWithUpload(FtpServer As FtpServer, DirectoryName As String, FileList As List(Of String),
        ...
        Try
            ...
            NLOGLOGGER.Info("Uploading Files with FTP ...")
            For Each FileItem As String In FileList

                If LocalDirectory.Length > 0 Then
                    NLOGLOGGER.Trace("Try to upload: " & FileItem)
                    FTPClnt.UploadFile(LocalDirectory & "\" & FileItem, DirectoryName & "/" & FileItem, FtpRemoteExists.Overwrite)
                    If FTPClnt.FileExists(DirectoryName & "/" & FileItem) = True Then
